...
# Map strain identifier in filename → condition
STRAIN_TO_CONDITION = {
    "SWF1578": "BlaC-Cat.Dead",
    "SWF1555": "BlaC",
}


def discover_animals(directory: Path, strain_map: dict) -> list:
    """Scan `directory` and build the ANIMALS list based on strain substrings."""
    animals = []
    seen = set()
    for entry in sorted(directory.iterdir()):
        name = entry.name
        matched = next((s for s in strain_map if s in name), None)
        if matched is None:
            continue
        # Strip .zarr (or any) extension to get the canonical animal name
        animal_name = name[:-len(".zarr")] if name.endswith(".zarr") else entry.stem
        if animal_name in seen:
            continue
        seen.add(animal_name)
        animals.append({
            "name": animal_name,
            "condition": strain_map[matched],
            "dataset": "new",
            "dir": directory,
        })
    return animals
...
ANIMALS = discover_animals(PREPROCESSED_DIR_NEW, STRAIN_TO_CONDITION)
...
for name in blac_cat_dead:
    ...
    ax.plot(0 + jitter, post_bleach_averages[name],
            "o", color=COLOR_DEAD, markersize=9, alpha=0.8,
            markeredgecolor="white", markeredgewidth=0.5)

for name in blac:
    jitter = np.random.uniform(-0.12, 0.12)
    ax.plot(0 + jitter, post_bleach_averages[name],
            "o", color=COLOR_ALIVE, markersize=9, alpha=0.8,
            markeredgecolor="white", markeredgewidth=0.5)
